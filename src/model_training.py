...
class ModelTrainer:
    ...
    def train(
        ...
        # Log target distribution (works for both numeric and categorical)
        try:
            if isinstance(Y_train, pd.Series):
                target_dist = Y_train.value_counts().to_dict()
            else:
                target_dist = pd.Series(Y_train).value_counts().to_dict()
            logger.info(f"  • Target Distribution: {target_dist}")
        except Exception as e:
            logger.warning(f"  • Target Distribution: Unable to compute ({str(e)})")
